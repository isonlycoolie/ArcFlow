...
```json
{
  ...
  "$defs": {
    ...
    "StepResult": {
      ...
      "properties": {
        ...
        "tokens_used": {
          "type": "integer",
          "minimum": 0,
          "description": "Token usage when reported. Sprint 5."
        }
      },
      "required": ["step_id", "status", "latency_ms"],
      "additionalProperties": false
    },
    "ErrorPayload": {
      "type": "object",
      "description": "Structured error returned to SDKs. Sprint 2â€“3, Sprint 7.",
      "properties": {
        "code": { "$ref": "#/$defs/ErrorCode" },
        "message": {
          "type": "string",
          "description": "Human-readable error message. Sprint 7+."
        },
        "step_id": {
          "$ref": "#/$defs/Uuid",
          "description": "Step associated with the error when applicable. Sprint 7."
        },
        "recoverable": {
          "type": "boolean",
          "description": "Whether the caller may retry. Sprint 7."
        }
      },
    ...
    "RunResult": {
      "type": "object",
      "description": "Final workflow execution outcome. Sprint 2â€“3, Sprint 5â€“7.",
      "properties": {
        "trace_id": { "$ref": "#/$defs/Uuid" },
        "status": { "$ref": "#/$defs/ExecutionStatus" },
        "output": {
          "type": "string",
          "description": "Final workflow output when successful. Sprint 2â€“3."
        },
        "steps": {
          "type": "array",
          "items": { "$ref": "#/$defs/StepResult" },
          "description": "Per-step results. Sprint 5â€“7."
        },
        "error": {
          "$ref": "#/$defs/ErrorPayload",
          "description": "Error details when status is Failed. Sprint 2â€“3, Sprint 7."
        }
      },
      "required": ["trace_id", "status", "steps"],
      "additionalProperties": false
    },
    "TraceEvent": {
      "type": "object",
      "description": "Observability event emitted during execution. Sprint 5.",
      "properties": {
        "trace_id": { "$ref": "#/$defs/Uuid" },
        "event_kind": { "$ref": "#/$defs/TraceEventKind" },
        "timestamp": { "$ref": "#/$defs/DateTimeUtc" },
        "step_id": {
          "$ref": "#/$defs/Uuid",
          "description": "Related step when applicable. Sprint 5."
        },
        "data": {
          "description": "Optional structured event payload. Sprint 5.",
          "type": "object"
        }
      },
      "required": ["trace_id", "event_kind", "timestamp"],
      "additionalProperties": false
    },
    "MessageEnvelope": {
      "type": "object",
      "description": "Wire wrapper for every RCS message. All sprints.",
      "properties": {
        "rcs_version": {
          "type": "string",
          "pattern": "^\\d+\\.\\d+$",
          "description": "Protocol version MAJOR.MINOR for negotiation. All sprints."
        },
        "message_type": { "$ref": "#/$defs/MessageType" },
        "trace_id": { "$ref": "#/$defs/Uuid" },
        "timestamp": { "$ref": "#/$defs/DateTimeUtc" },
        "payload": {
          "description": "Typed message body as JSON. All sprints.",
          "type": "object"
        }
      },
      "required": ["rcs_version", "message_type", "trace_id", "timestamp", "payload"],
      "additionalProperties": false
    }
  }
}
