...
Optional `fallback_step_id` on a step routes to a backup agent on primary failure (`StepFallbackActivated` in trace). Step-level and workflow-level retry policies live in RCS and `exec_config`.
...
Checkpoints: when recovery is enabled, each node triggers `persist_graph_checkpoint` with `current_node_id` and `graph_iteration_count`. **Full resume dispatch from graph checkpoints is partial (FP-1.01):** schema and persist path exist; resuming mid-graph after failure is not production-complete.
...
Memory reads and vector retrieval emit SEC-1 safe events (`MemoryRetrieved` records `chunk_count` and `total_bytes` only).
