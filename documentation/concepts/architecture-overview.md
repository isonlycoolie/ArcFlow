...
Below that is **workflow specification JSON**: workflow definitions, agent definitions, run requests, and `exec_config`. Normative schemas are documented in [workflow schema](../contracts/rcs-schema.md). See [Workflow specification](the-rcs-contract.md) for how contract-first design keeps SDK and HTTP behavior aligned.
...
4. The browser polls trace via Relay: `GET /v1/sites/{site_id}/runs/{run_id}/trace`, which proxies to `GET /v1/runs/{id}/trace` on the server. Events are metadata-only (trace data policy).
...
3. During execution the engine writes recovery checkpoints and trace events to Postgres when `recovery_enabled` is true. Linear recovery can resume from the last committed step index; graph checkpoint fields exist but full resume dispatch is partial (Graph recovery resume).
...
| Trace field rules | [Trace data policy](sec-1-and-data-safety.md) |
