...
2. Events are typed structs — never opaque `data` / `value` / `extra` bags.
...
| run_id, step_id, tool_name | string | Identifiers |
| duration_ms | u64 | Elapsed |
| failure_reason | string | `execution_error` \| `timeout` \| … |
| error_code | string | Sanitized code |
...
- [ ] CAA — schema complete, SEC-1 satisfied  
- [ ] OBA — field names semantic, granularity approved  
- [ ] QAA — test matrix can be derived from this document  
