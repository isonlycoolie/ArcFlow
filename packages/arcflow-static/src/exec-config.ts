...
export interface ExecConfigPayload {
  retry?: { max_attempts: number; backoff: Record<string, unknown> };
...
export function buildExecConfig(options: {
  retry?: RetryOptions;
    ...
    payload.retry = {
      ...
      backoff:
        backoff.kind === "constant"
          ? { kind: "constant", delay_ms: backoff.delayMs ?? 100, jitter_ms: backoff.jitterMs ?? 0 }
          : backoff.kind === "linear"
            ? {
                kind: "linear",
  ...
  if (options.recoveryEnabled) payload.recovery_enabled = true;
  if (options.stream) payload.stream = { enabled: true };
  if (options.initialState) payload.initial_state = options.initialState;
  return Object.keys(payload).length ? (payload as Record<string, unknown>) : undefined;
}
